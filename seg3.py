...
def get_index(f) :
    ...
    for i in f :
        ...
        if i == float(1000) or i == float(2000) or i == float(3000) or i == float(4000) or  i == float(5000) :
            iArr.append(c)   
            c = c+1
        elif i < float(2000) and i != float(1000) : # 저주파대 예외처리
            iArr2.append(c)
            c = c+1
        else :
            c = c+1 
...
def find_mValue(ratioArr) :
    ...
    for value in ratioArr :
        ...
        if (det1 == 0) and (det2 == 5) and (value >= float(300)) :
            IsBeep = True
...
def set_files(path) :
    ...
    for root, dirs, files in os.walk(path) :
        for f in files :
            ...
            if (ext == '.wav') and (f != 'beep_processed.wav') and (f != '000join_output.wav') :
                infiles.append(f)
...
def seg_corpus(mValueArr, infiles, fileName) :
    n = 0
    p1 = 0
    p2 = 0
    length = 0
...
print ": find mValue"
seg_corpus(mValueArr, infiles, fileName)
print ": seg corpus"
print ": completed"
